...
SCRIPT_LOG_FILE = f"{LOGS_PATH}/report_balances.csv"
lsb_logger = None
...
class ReportBalanceStatus(ScriptStrategyBase, MarketsYmlConfig):
    ...
    async def _async_calls(self) -> None:
        ...
        for exchange_name in self.connectors.keys():
            if tasks_df[tasks_df.func_name == 'update_exchange_balance'].empty:
                await UserBalances.instance().update_exchange_balance(exchange_name)
            if exchange_name == 'kucoin':
                if tasks_df[tasks_df.func_name == 'get_kucoin_prices'].empty:
                    self._prices[exchange_name] = await RateOracle.get_kucoin_prices()
            elif exchange_name == 'gate_io':
                if tasks_df[tasks_df.func_name == 'get_gate_io_prices'].empty:
                    self._prices[exchange_name] = await RateOracle.get_gate_io_prices()
            else:
                self.logger().error(f"{exchange_name} is not implemented")
                raise ValueError
    ...
    def _implement_buys_sells(balance_sheet: pd.DataFrame) -> pd.DataFrame:
        ...
        if list(sells.values())[0]['Funding Gap (USDT)'] >= 1 or list(sells.values())[0]['Funding Gap (USDT)'] > 1:
            ...
            ReportBalanceStatus.logger().error("There is an error in matching buys and sells")
            raise ValueError
