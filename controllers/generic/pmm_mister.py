...
class PMMisterConfig(ControllerConfigBase):
    ...
    def update_parameters(
        self, trade_type: TradeType, new_spreads: list[float] | str, new_amounts_pct: list[int] | str | None = None
    ):
...
class PMMister(ControllerBase):
    ...
    def _verify_position_mode(self) -> bool:
        ...
        try:
            ...
            if exchange_mode != config_mode:
                ...
                return False
            self._position_mode_verified = True
            self.logger().info(f"Position mode verified: {exchange_mode} matches config. Trading enabled.")
            return True
    ...
    def _check_global_tp_sl(self) -> list[ExecutorAction]:
        ...
        if self._global_close_phase == "stopping":
            active_non_close = [
                e for e in self.executors_info if e.is_active and e.custom_info.get("level_id") != "global_close"
            ]
            if active_non_close:
                self.logger().debug(
        ...
        position_held = next(
            (
                p
                for p in self.positions_held
                if p.trading_pair == self.config.trading_pair and p.connector_name == self.config.connector_name
            ),
            None,
        )
    ...
    def _update_position_state(self):
        ...
        if position_held is not None:
            ...
            if self.config.global_pnl_reference == "portfolio":
                ...
                pnl_denominator = (
                    abs(position_amount) * breakeven_price
                    if breakeven_price and breakeven_price > 0
                    else abs(position_held.amount_quote)
                )
            unrealized_pnl_pct = (
                position_held.unrealized_pnl_quote / pnl_denominator if pnl_denominator != 0 else Decimal("0")
            )
        else:
        ...
        executor_fees = sum((e.cum_fees_quote for e in self.executors_info if e.is_active), Decimal("0"))
    ...
    def get_custom_info(self) -> dict:
        ...
        can_sell = sum(
            1 for lc in level_conditions.values() if lc.get("trade_type") == "SELL" and lc.get("can_execute")
        )
    ...
    def to_format_status(self) -> list[str]:
        ...
        tp_status = (
            "ACTIVE"
            if tp_active
            else ("OFF" if not self.config.global_tp_enabled else f"from {self.config.global_tp_activation_from}")
        )
        sl_status = (
            "ACTIVE"
            if sl_active
            else ("OFF" if not self.config.global_sl_enabled else f"from {self.config.global_sl_activation_from}")
        )
    ...
    def _format_position_visualization(
        ...
        lines.append(
            f"│ Position PnL: [{pnl_bar}] {pnl_status} (S={-self.config.global_stop_loss:.2%} T={self.config.global_take_profit:.2%}) │"
        )
