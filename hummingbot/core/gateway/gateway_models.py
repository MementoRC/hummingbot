...
class AmmAddLiquidityResponseData(BaseModel):
    ...
    position_rent: Decimal | None = Field(
        None,
        alias="positionRent",
...
class AmmRemoveLiquidityResponseData(BaseModel):
    ...
    position_rent_refunded: Decimal | None = Field(
        None,
        alias="positionRentRefunded",
...
class PositionDetail(BaseModel):
    position_address: str = Field(
        ..., alias="positionAddress", description="Address of the individual position (NFT position account)"
    )
    lp_token_amount: Decimal = Field(
        ..., alias="lpTokenAmount", description="Liquidity held by this position (LP units)"
    )
    base_token_amount: Decimal = Field(..., alias="baseTokenAmount")
    quote_token_amount: Decimal = Field(..., alias="quoteTokenAmount")
...
class BalanceRequest(BaseModel):
    ...
    fetch_all: bool | None = Field(
        None, alias="fetchAll", description="fetch all tokens in wallet, not just those in token list (default: false)"
    )
...
class ChainQuoteSwapResponse(BaseModel):
    ...
    price_impact_pct: Decimal = Field(
        ..., alias="priceImpactPct", description="Estimated price impact percentage (0-100)"
    )
    min_amount_out: Decimal = Field(
        ..., alias="minAmountOut", description="Minimum amount of tokenOut that will be accepted"
    )
...
class ChainExecuteSwapResponseData(BaseModel):
    ...
    base_token_balance_change: Decimal = Field(
        ..., alias="baseTokenBalanceChange", description="Change in base token balance (negative for decrease)"
    )
    quote_token_balance_change: Decimal = Field(
        ..., alias="quoteTokenBalanceChange", description="Change in quote token balance (negative for decrease)"
    )
    slippage_pct: Decimal | None = Field(
        None, alias="slippagePct", description="Slippage tolerance percentage actually applied to the swap"
    )
...
class WrapRequest(BaseModel):
    ...
    amount: str = Field(
        ..., description="Amount of the native token to wrap, in whole units (not lamports/wei)", examples=["1.0"]
    )
...
class AmmCreatePoolRequest(BaseModel):
    ...
    quote_token: str = Field(
        ..., alias="quoteToken", description="Quote token symbol or address (becomes the pool quote)"
    )
    base_token_amount: Decimal = Field(
        ..., alias="baseTokenAmount", description="Amount of base token to seed the pool with"
    )
    quote_token_amount: Decimal | None = Field(
        None,
        alias="quoteTokenAmount",
    ...
    initial_price: Decimal | None = Field(
        None,
        alias="initialPrice",
    ...
    amm_config_index: float | None = Field(
        None,
        alias="ammConfigIndex",
        description="Raydium CPMM fee-config index (optional; defaults to the first available config).",
    )
    slippage_pct: condecimal(ge=Decimal("0"), le=Decimal("100")) | None = Field(
        None,
        alias="slippagePct",
...
class AmmRemoveRequest(BaseModel):
    ...
    percentage_to_remove: condecimal(ge=Decimal("0"), le=Decimal("100")) = Field(
        ..., alias="percentageToRemove", description="Percentage of liquidity to remove", examples=[100]
    )
    slippage_pct: condecimal(ge=Decimal("0"), le=Decimal("100")) | None = Field(
        None,
        alias="slippagePct",
...
class AmmPositionsOwnedRequest(BaseModel):
    ...
    connector: str = Field(
        ..., description="AMM connector (only non-fungible-LP AMMs supported: meteora)", examples=["meteora"]
    )
    chain_network: str = Field(
        ...,
        alias="chainNetwork",
...
class ClmmOpenRequest(BaseModel):
    ...
    lower_price: Decimal = Field(
        ..., alias="lowerPrice", description="Lower price bound for the position", examples=[150]
    )
    upper_price: Decimal = Field(
        ..., alias="upperPrice", description="Upper price bound for the position", examples=[250]
    )
    pool_address: str = Field(
        ..., alias="poolAddress", description="Pool address", examples=["2sf5NYcY4zUPXUSmG6f66mskb24t5F8S11pC1Nz5nQT3"]
    )
    base_token_amount: Decimal | None = Field(
        None, alias="baseTokenAmount", description="Amount of base token to deposit", examples=[0.01]
    )
    quote_token_amount: Decimal | None = Field(
        None, alias="quoteTokenAmount", description="Amount of quote token to deposit", examples=[2]
    )
    slippage_pct: condecimal(ge=Decimal("0"), le=Decimal("100")) | None = Field(
        None,
        alias="slippagePct",
    ...
    strategy_type: float | None = Field(
        None,
        alias="strategyType",
...
class ClmmAddRequest(BaseModel):
    ...
    wallet_address: str = Field(..., alias="walletAddress", description="Wallet address")
    position_address: str = Field(
        ..., alias="positionAddress", description="Position address", examples=["<sample-position-address>"]
    )
    base_token_amount: Decimal | None = Field(
        None,
        alias="baseTokenAmount",
...
class ClmmFetchPoolsRequest(BaseModel):
    ...
    page: confloat(ge=0.0) | None = Field(
        None, description="0-based page index. Only connectors whose API paginates honor this."
    )
...
class ClmmPoolInfoRequest(BaseModel):
    ...
    bin_count: conint(ge=0, le=401) | None = Field(
        0,
        alias="binCount",
...
class RouterExecuteQuoteRequest(BaseModel):
    ...
    connector: str | None = Field(
        None, description="Router connector. Defaults to the network's swapProvider", examples=["jupiter"]
    )
...
class RouterExecuteSwapRequest(BaseModel):
    ...
    approximate_if_no_exact_out: bool | None = Field(
        True,
        alias="approximateIfNoExactOut",
...
class RouterQuoteSwapRequest(BaseModel):
    ...
    indicative_price: bool | None = Field(
        None,
        alias="indicativePrice",
...
class AmmQuoteSwapRequest(BaseModel):
    ...
    connector: str | None = Field(
        "meteora", description="AMM connector to price the swap against", examples=["meteora"]
    )
    base_token: str = Field(..., alias="baseToken", description="Symbol or address of the base token")
    quote_token: str = Field(..., alias="quoteToken", description="Symbol or address of the quote token")
    amount: Decimal = Field(..., description="Amount of base token to trade")
...
class ClmmQuoteSwapRequest(BaseModel):
    ...
    connector: str | None = Field(
        "meteora", description="CLMM connector to price the swap against", examples=["meteora"]
    )
    base_token: str = Field(..., alias="baseToken", description="Symbol or address of the base token")
    quote_token: str = Field(..., alias="quoteToken", description="Symbol or address of the quote token")
    amount: Decimal = Field(..., description="Amount of base token to trade")
...
class AmmExecuteSwapRequest(BaseModel):
    ...
    connector: str | None = Field(
        "meteora", description="AMM connector to execute the swap against", examples=["meteora"]
    )
    wallet_address: str = Field(..., alias="walletAddress", description="Wallet address that will execute the swap")
    base_token: str = Field(..., alias="baseToken", description="Symbol or address of the base token")
    quote_token: str = Field(..., alias="quoteToken", description="Symbol or address of the quote token")
...
class ClmmExecuteSwapRequest(BaseModel):
    ...
    connector: str | None = Field(
        "meteora", description="CLMM connector to execute the swap against", examples=["meteora"]
    )
    wallet_address: str = Field(..., alias="walletAddress", description="Wallet address that will execute the swap")
    base_token: str = Field(..., alias="baseToken", description="Symbol or address of the base token")
    quote_token: str = Field(..., alias="quoteToken", description="Symbol or address of the quote token")
...
class AddHardwareWalletRequest(BaseModel):
    ...
    set_default: bool | None = Field(
        False, alias="setDefault", description="Set this wallet as the default for the chain"
    )
...
class Token(BaseModel):
    ...
    address: str = Field(
        ..., description="The token contract address", examples=["0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"]
    )
