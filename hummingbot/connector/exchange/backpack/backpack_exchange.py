    ...
    async def _update_balances(self):
        ...
        if account_info:
            for asset_name, balance_entry in account_info.items():
                ...
                total_balance = (
                    Decimal(balance_entry["available"])
            ...
            for asset_name, lent_amount in lent_balances.items():
                ...
                self._account_balances[asset_name] = self._account_balances.get(asset_name, Decimal("0")) + lent_amount
                remote_asset_names.add(asset_name)
    ...
    async def _get_net_lent_balances(self) -> dict[str, Decimal]:
        ...
        try:
            ...
            self.logger().warning(
                ...
                "under-reported until the next successful update.",
                exc_info=True,
            )
