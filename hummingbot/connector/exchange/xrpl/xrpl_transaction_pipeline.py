        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    if not future.done():
                        # Clear the traceback before storing on the future.
                        # In Python 3.12, a traceback that references this running
                        # coroutine frame can interfere with task cancellation,
                        # causing "cannot reuse already awaited coroutine" when
                        # stop() awaits self._pipeline_task.
