...
class CLMMPoolInfo(BaseModel):
    ...
    bin_step: int | None = Field(default=None, alias="binStep")
    fee_pct: float = Field(alias="feePct")
    price: float
    base_token_amount: float = Field(alias="baseTokenAmount")
    quote_token_amount: float = Field(alias="quoteTokenAmount")
...
class AMMPositionInfo(BaseModel):
    ...
    positions: list[AMMPositionDetail] | None = None
...
class Gateway(GatewayBase):
    ...
    def _parse_dex_name(dex_name: str) -> tuple:
        ...
        if "/" not in dex_name:
            raise ValueError(
                f"Invalid swap provider '{dex_name}' - expected 'name/type' (e.g. 'jupiter/router', 'meteora/clmm')"
            )
    ...
    async def get_quote_price(
        self, trading_pair: str, is_buy: bool, amount: Decimal, slippage_pct: Decimal | None = None
    ) -> Decimal | None:
        """
