...
class ExchangeBase(ConnectorBase):
    ...
    def c_get_vwap_for_volume(self, trading_pair: str, is_buy: bool, volume: Decimal) -> ClientOrderBookQueryResult:
        ...
        return ClientOrderBookQueryResult(s_decimal_NaN, query_volume, result_price, result_volume)

    def c_get_price_for_quote_volume(
    ...
    def c_get_quote_volume_for_base_amount(
    ...
    def c_get_volume_for_price(self, trading_pair: str, is_buy: bool, price: Decimal) -> ClientOrderBookQueryResult:
        ...
        result_price = Decimal(str(result.result_price))
        result_volume = Decimal(str(result.result_volume))
        return ClientOrderBookQueryResult(query_price, s_decimal_NaN, result_price, result_volume)

    def c_get_quote_volume_for_price(
    ...
    def order_book_bid_entries(self, trading_pair: str) -> Iterator[ClientOrderBookRow]:
        order_book: OrderBook = self.c_get_order_book(trading_pair)
        for entry in order_book.bid_entries():
            yield ClientOrderBookRow(Decimal(str(entry.price)), Decimal(str(entry.amount)), entry.update_id)

    def order_book_ask_entries(self, trading_pair: str) -> Iterator[ClientOrderBookRow]:
        order_book: OrderBook = self.c_get_order_book(trading_pair)
        for entry in order_book.ask_entries():
            yield ClientOrderBookRow(Decimal(str(entry.price)), Decimal(str(entry.amount)), entry.update_id)

    def get_vwap_for_volume(self, trading_pair: str, is_buy: bool, volume: Decimal) -> ClientOrderBookQueryResult:
        return self.c_get_vwap_for_volume(trading_pair, is_buy, volume)

    def get_price_for_quote_volume(
        self, trading_pair: str, is_buy: bool, volume: Decimal
    ) -> ClientOrderBookQueryResult:
        return self.c_get_price_for_quote_volume(trading_pair, is_buy, volume)

    def get_price_for_volume(self, trading_pair: str, is_buy: bool, volume: Decimal) -> ClientOrderBookQueryResult:
