...
def _resolve_strategy_type(strategy: str, v1: bool, v2: bool, controller: bool) -> str:
    ...
    if len(matches) > 1:
        fail(
            f"'{strategy}' exists as {' and '.join(matches)} — disambiguate with "
            f"{' / '.join('--' + m for m in matches)}",
            ExitCode.CONFIG_ERROR,
        )
    avail = {t: available_sources(t) for t in STRATEGY_TYPES}
    hint = "; ".join(
        f"{t}: {', '.join(avail[t][:8])}{' …' if len(avail[t]) > 8 else ''}" for t in STRATEGY_TYPES if avail[t]
    )
...
def create_config(
    *,
    ...
    if collisions:
        ...
        if name:
            fail(
                f"'{out_name}' already exists as a {' and '.join(collisions)} config — config names must "
                f"be unique across types. Try --name {suggestion}",
                ExitCode.CONFIG_ERROR,
            )
        out_name = suggestion
    ...
    if remaining and not with_defaults:
        fail(
            f"missing required fields: {', '.join(remaining)}. Supply them with --set key=value "
    ...
    record = {"file": out_name, "type": stype, "applied": ", ".join(sorted(values)) or "-", "ready": not remaining}
    if remaining:
