...
async def _compute(fills: list, balances: dict[str, dict[str, float]]) -> list[dict]:
    ...
    for (market, symbol), trades in groups.items():
        ...
        try:
            perf = await asyncio.wait_for(PerformanceMetrics.create(symbol, trades, cur_balances), PERF_TIMEOUT)
        except Exception as e:
            # str() of e.g. asyncio.TimeoutError is empty — always name the exception type.
            results.append(
                {"market": market, "pair": symbol, "trades": len(trades), "error": str(e) or type(e).__name__}
            )
            continue
        results.append(
            {
