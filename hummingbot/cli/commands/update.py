...
def _git(*args: str) -> str:
    ...
    try:
        proc = subprocess.run(["git", *args], cwd=REPO_ROOT, capture_output=True, text=True, timeout=GIT_TIMEOUT)
    ...
    if proc.returncode != 0:
        fail(f"git {' '.join(args)} failed: {proc.stderr.strip() or proc.stdout.strip()}", ExitCode.ERROR)
...
def _rebuild_extensions() -> None:
    ...
    proc = subprocess.run([sys.executable, "setup.py", "build_ext", "--inplace", "-j", "8"], cwd=REPO_ROOT)
    if proc.returncode != 0:
        fail(
            "extension build failed — the checkout is updated but NOT rebuilt; "
            "fix the build (see output above) or run `make install`",
            ExitCode.ERROR,
        )
...
def update(
    ...
    if os.environ.get("INSTALLATION_TYPE") == "docker":
        fail(
            "this is a Docker install — a container cannot replace its own image. "
    ...
    if not (REPO_ROOT / ".git").exists():
        fail(
            f"{REPO_ROOT} is not a git checkout — `hbot update` only knows how to update "
    ...
    if ahead > 0:
        fail(
            f"local branch has {ahead} commit(s) the upstream lacks — refusing to guess; "
            f"update manually (e.g. `git pull --rebase`) and rerun",
            ExitCode.ERROR,
        )
    ...
    record: dict = {
        "version": f"{old_version} -> {_version()}",
        "branch": branch,
        "updated": f"{local} -> {_git('rev-parse', '--short', 'HEAD')}",
        "commits": behind,
        "extensions_rebuilt": rebuilt,
    }
    if env_changed:
