...
class OrderTracker(TimeIterator):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._tracked_limit_orders: Dict = {}
        self._tracked_market_orders: Dict = {}
        self._order_id_to_market_pair: Dict = {}
        self._shadow_tracked_limit_orders: Dict = {}
        self._shadow_order_id_to_market_pair: Dict = {}
        self._shadow_gc_requests: deque = deque()
        self._in_flight_pending_created: set = set()
        self._in_flight_cancels: OrderedDict = OrderedDict()
    ...
    def tracked_limit_orders(self) -> List[Tuple[ConnectorBase, LimitOrder]]:
        return [
            (market_trading_pair_tuple[0], order)
    ...
    def tracked_limit_orders_data_frame(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            data=limit_orders,
            columns=["market", "trading_pair", "order_id", "quantity", "timestamp"],
        )
    ...
    def get_limit_orders(self) -> Dict:
        return self.c_get_limit_orders()

    def c_get_market_orders(self) -> Dict:
        return self._tracked_market_orders

    def get_market_orders(self) -> Dict:
        return self.c_get_market_orders()

    def c_get_shadow_limit_orders(self) -> Dict:
    ...
    def c_has_in_flight_cancel(self, order_id: str) -> bool:
    ...
    def c_check_and_track_cancel(self, order_id: str) -> bool:
        """
        ...
        if order_id in self._in_flight_pending_created:
            return False
    ...
    def c_get_market_pair_from_order_id(self, order_id: str):
    ...
    def c_get_shadow_market_pair_from_order_id(self, order_id: str):
    ...
    def c_get_limit_order(self, market_pair, order_id: str) -> LimitOrder:
    ...
    def c_get_market_order(self, market_pair, order_id: str):
    ...
    def c_get_shadow_limit_order(self, order_id: str) -> LimitOrder:
        market_pair = self._shadow_order_id_to_market_pair.get(order_id)
    ...
    def c_check_and_cleanup_shadow_records(self) -> None:
        ...
        while len(self._shadow_gc_requests) > 0 and self._shadow_gc_requests[0][0] < current_timestamp:
            ...
            if (
                market_pair in self._shadow_tracked_limit_orders
                and order_id in self._shadow_tracked_limit_orders[market_pair]
            ):
    ...
    def check_and_cleanup_shadow_records(self) -> None:
        self.c_check_and_cleanup_shadow_records()

    def c_add_create_order_pending(self, order_id: str) -> None:
        self.in_flight_pending_created.add(order_id)

    def add_create_order_pending(self, order_id: str) -> None:
        self.c_add_create_order_pending(order_id)

    def c_remove_create_order_pending(self, order_id: str) -> None:
        self._in_flight_pending_created.discard(order_id)

    def remove_create_order_pending(self, order_id: str) -> None:
