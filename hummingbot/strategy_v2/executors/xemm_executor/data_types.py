...
class XEMMExecutorConfig(ExecutorConfigBase):
    ...
    def validate_xemm(self):
        ...
        require_not_above(
            "min_profitability", self.min_profitability, "target_profitability", self.target_profitability
        )
        require_not_above(
            "target_profitability", self.target_profitability, "max_profitability", self.max_profitability
        )
        if self.min_profitability == self.max_profitability:
            raise ValueError(
                f"min_profitability ({self.min_profitability}) and max_profitability "
                f"({self.max_profitability}) must define a non empty band"
            )
        if self.buying_market == self.selling_market:
            raise ValueError(
                f"buying_market and selling_market must be different markets, both are "
        ...
        require_interchangeable_pairs(
            "buying_market.trading_pair",
            self.buying_market.trading_pair,
            "selling_market.trading_pair",
