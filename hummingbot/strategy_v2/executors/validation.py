...
def require_non_empty(field: str, value: str | None) -> None:
    """Validate that a string field is set and not blank."""
    if value is None or not value.strip():
        raise ValueError(f"{field} must not be empty")


def require_positive(field: str, value: Number | None) -> None:
...
def require_non_negative(field: str, value: Number | None) -> None:
    """Validate that a value is not negative. ``None`` is skipped."""
    if value is not None and value < 0:
        raise ValueError(f"{field} ({value}) must be greater than or equal to 0")


def require_at_least(field: str, value: Number | None, minimum: Number) -> None:
...
def require_lower_than(field: str, value: Number | None, other_field: str, other_value: Number | None) -> None:
...
def require_not_above(field: str, value: Number | None, other_field: str, other_value: Number | None) -> None:
...
def require_all_positive(field: str, values: Sequence[Number] | None) -> None:
...
def require_trading_pair(field: str, trading_pair: str | None) -> None:
...
def require_provider(field: str, provider: str | None, required: bool = True) -> None:
    """
...
def require_stop_price(side: TradeType, field: str, value: Decimal | None, boundaries: Sequence[NamedValue]) -> None:
    """
