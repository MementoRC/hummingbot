...
class BacktestingEngineBase:
    ...
    def collect_executors_ledger(self, timestamp: float) -> List[ExecutorInfo]:
        ...
        """
        still_simulating = [
            simulation.get_executor_info_at_timestamp(timestamp) for simulation in self.active_executor_simulations
        ]
    ...
    def summarize_results(
        ...
        if len(executors_info) > 0:
            ...
            if pnl_timeseries and len(pnl_timeseries) > 1:
                ...
                non_hold_with_position["cumulative_volume"] = (
                    non_hold_with_position["filled_amount_quote"].astype(float).cumsum()
                )
