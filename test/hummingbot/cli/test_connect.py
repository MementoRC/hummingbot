...
def _field(attr, secure=False, prompt="enter"):
    ...
    return SimpleNamespace(
        attr=attr, client_field_data=SimpleNamespace(is_connect_key=True, is_secure=secure, prompt=prompt)
    )
...
class ConnectCommandTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        patch("hummingbot.cli.commands.connect._connectable_exchanges", return_value=["binance", "kraken"]).start()
        ...
        patch("hummingbot.client.config.config_helpers.load_client_config_map_from_file", return_value=self.ccm).start()
        self.addCleanup(patch.stopall)

    def _run(self, connector=None, **kw) -> str:
        args = dict(keys_stdin=False, replace=False, show_fields=False, show_all=False, password_stdin=False)
        args.update(kw)
        buf = io.StringIO()
    ...
    def test_no_arg_tests_connected_keys(self):
        ...
        with (
            patch("hummingbot.cli.commands.connect.resolve_password", return_value="pw"),
    ...
    def test_add_keys_from_stdin(self):
        with patch(
            "hummingbot.cli.commands._common.read_json_object_from_stdin",
    ...
    def test_add_keys_stdin_missing_fields_fails(self):
        with patch(
            "hummingbot.cli.commands._common.read_json_object_from_stdin", return_value={"binance_api_key": "k"}
        ):
    ...
    def test_add_keys_via_tty_prompts(self):
        with patch("sys.stdin.isatty", return_value=True), patch("getpass.getpass", side_effect=["k", "s"]):
    ...
    def test_rejected_key_value_fails(self):
        ...
        with (
            patch("hummingbot.client.settings.AllConnectorSettings") as acs,
            patch("hummingbot.client.config.config_helpers.ClientConfigAdapter", return_value=cfg),
            patch("hummingbot.cli.commands._common.read_json_object_from_stdin", return_value={"api_key": "k"}),
        ):
