...
class ResolveTargetTest(unittest.TestCase):
    def test_existing_config_file_wins(self):
        ...
        with patch.object(
            sc, "matching_config_types", side_effect=lambda fn: ["controller"] if fn == "conf_x.yml" else []
        ):
    ...
    def test_strategy_name_when_no_config_matches(self):
        with (
            patch.object(sc, "matching_config_types", return_value=[]),
...
class DeployCommandTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        launch_patch = patch("hummingbot.cli.commands.start.launch", return_value={"state": "running", "pid": 4242})
    ...
    def test_existing_config_stdin_values_applied(self):
        path = self.dirs["v2-script"] / "conf_x.yml"
        path.write_text("a: 1\n")
        with patch("hummingbot.cli.commands._common.read_json_object_from_stdin", return_value={"a": 7}):
    ...
    def test_strategy_name_creates_a_config_then_starts(self):
        created = {"file": "conf_pmm.yml", "type": "controller", "applied": "a, b", "ready": True, "next": "hbot start"}
        out = io.StringIO()
        with (
            patch("hummingbot.cli.commands.deploy.resolve_target", return_value=("strategy", "pmm_simple", None)),
            patch("hummingbot.cli.commands.create.create_config", return_value=created) as cc,
        ):
            with redirect_stdout(out):
                run_deploy("pmm_simple", set_values=["a=1", "b=2"], controller=True, password_stdin=True)
