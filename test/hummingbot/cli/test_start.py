...
class LogTailTest(unittest.TestCase):
    def test_combines_both_logs_and_keeps_last_n(self):
        with (
            patch.object(bot, "structured_log_file", return_value=Path("/s.log")),
...
class LaunchTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.spawn = self.stack.enter_context(
            patch(
                "hummingbot.cli.commands.start._spawn_detached",
    ...
    def test_v1_strategy_uses_config_flag(self):
        ...
        self.assertEqual(
            cmd, [sys.executable, "-m", "hummingbot.cli.engine", "--name", "conf_v1", "--config", "conf_v1.yml"]
        )
        ...
        self.assertEqual(
            (meta["name"], meta["type"], meta["file"], meta["config"], meta["script_config"]),
    ...
    def test_controller_is_wrapped_in_a_v2_loader(self):
        with (
            patch.object(sc, "resolve_config_type", return_value="controller"),
    ...
    def test_no_file_runs_the_loaded_config_with_its_recorded_type(self):
        with (
            patch.object(bot, "read_loaded", return_value={"file": "conf_s.yml", "type": "v2-script"}),
    ...
    def test_already_running_without_replace_fails(self):
        with (
            patch.object(sc, "resolve_config_type", return_value="v2-script"),
    ...
    def test_foreground_execs_the_engine_in_place(self):
        ...
        with (
            patch.object(sc, "resolve_config_type", return_value="v1-strategy"),
...
class SpawnDetachedTest(unittest.TestCase):
    ...
    def test_polls_until_the_strategy_is_running(self):
        ...
        self.proc.poll.return_value = None
        with patch.object(bot, "read_status", side_effect=[None, {"engine": {"strategy_running": True}}]):
            record = start_mod._spawn_detached(self.cmd, self.env, "n", 60.0)
...
class StartCommandTest(unittest.TestCase):
    ...
    def _run(self, as_json):
        buf = io.StringIO()
        with (
            patch("hummingbot.cli.commands.start.launch", return_value=dict(self.RECORD)) as launch,
            redirect_stdout(buf),
        ):
            start_mod.start(
                file="conf_pmm.yml",
