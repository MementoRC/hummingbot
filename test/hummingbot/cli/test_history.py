...
def perf(return_pct="0.05"):
    return SimpleNamespace(
        ...
        num_buys=1,
        num_sells=1,
        tot_vol_base=Decimal("1.5"),
        tot_vol_quote=Decimal("3000"),
        trade_pnl=Decimal("10"),
        fee_in_quote=Decimal("1"),
        total_pnl=Decimal("9"),
...
class HistoryCommandTest(unittest.TestCase):
    def _run(
        self, fills, *, name=None, days=None, balances_status=None, running=False, perf_create=None, resolved=None
    ):
        resolved = resolved or ("/tmp/db.sqlite", "conf_x.yml", running)
        perf_create = perf_create or AsyncMock(return_value=perf())
        with (
            patch("hummingbot.cli.commands._common.resolve_db_for_command", return_value=resolved) as resolve_mock,
    ...
    def test_two_markets_with_balances_and_averaged_return(self):
        fills = [fill("binance", "BTC-USDT"), fill("binance", "BTC-USDT"), fill("kucoin", "ETH-USDT")]
    ...
    def test_running_hint_wording(self):
        ...
        with patch("hummingbot.cli.commands.status._request_fresh_snapshot"):
            printed, _, _ = self._run(
                fills, balances_status=status, running=True, resolved=("/tmp/db.sqlite", None, True)
            )
    ...
    def test_running_with_no_cached_balances_requests_snapshot(self):
        ...
        with (
            patch(
                "hummingbot.cli.commands._common.resolve_db_for_command", return_value=("/tmp/db.sqlite", None, True)
            ),
            patch("hummingbot.cli.data.get_trades", return_value=fills),
            patch("hummingbot.client.performance.PerformanceMetrics.create", AsyncMock(return_value=perf())),
    ...
    def test_named_bot_skips_balances_and_titles_table(self):
        ...
        with (
            patch(
                "hummingbot.cli.commands._common.resolve_db_for_command", return_value=("/tmp/past.sqlite", None, False)
            ) as resolve_mock,
            patch("hummingbot.cli.data.get_trades", return_value=fills) as get_trades,
            patch("hummingbot.client.performance.PerformanceMetrics.create", AsyncMock(return_value=perf())),
