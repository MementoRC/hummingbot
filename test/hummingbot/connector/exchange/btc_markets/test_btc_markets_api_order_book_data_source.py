...
class BtcMarketsAPIOrderBookDataSourceTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_last_traded_prices(self, mock_api):
        ...
        ret = await self.async_run_with_timeout(
    ...
    async def test_get_new_order_book_successful(self, mock_get):
        ...
        order_book = await self.async_run_with_timeout(coroutine=self.data_source.get_new_order_book(self.trading_pair))
    ...
    async def test_subscribe_channels_raises_cancel_exception(self):
        ...
        with self.assertRaises(asyncio.CancelledError):
            self.listening_task = asyncio.create_task(self.data_source._subscribe_channels(mock_ws))
            await self.async_run_with_timeout(self.listening_task)
    ...
    async def test_listen_for_trades_cancelled_when_listening(self):
        ...
        with self.assertRaises(asyncio.CancelledError):
            self.listening_task = asyncio.create_task(
                self.data_source.listen_for_trades(None, msg_queue)
            )
            await self.async_run_with_timeout(self.listening_task)
    ...
    async def test_listen_for_trades_successful(self):
        ...
        try:
            self.listening_task = asyncio.create_task(
                self.data_source.listen_for_trades(asyncio, msg_queue)
            )
        except asyncio.CancelledError:
            pass
    ...
    async def test_listen_for_order_book_diffs_cancelled(self):
        ...
        with self.assertRaises(asyncio.CancelledError):
            self.listening_task = asyncio.create_task(
                self.data_source.listen_for_order_book_diffs(asyncio, msg_queue)
            )
            await self.async_run_with_timeout(self.listening_task)
    ...
    async def test_listen_for_order_book_snapshots_cancelled_when_fetching_snapshot(self, mock_api, sleep_mock):
        ...
        with self.assertRaises(asyncio.CancelledError):
            self.listening_task = asyncio.create_task(
                self.data_source.listen_for_order_book_snapshots(asyncio, msg_queue)
            )
            await self.async_run_with_timeout(self.listening_task)
    ...
    async def test_listen_for_order_book_snapshots_log_exception(self, mock_api, sleep_mock):
        ...
        try:
            await self.async_run_with_timeout(self.data_source.listen_for_order_book_snapshots(asyncio, msg_queue))
        except asyncio.CancelledError:
            pass
    ...
    async def test_listen_for_order_book_snapshots_successful_ws(self, mock_api):
        ...
        msg: OrderBookMessage = await self.async_run_with_timeout(msg_queue.get(), timeout=6)
    ...
    async def test_order_book_snapshot(self, mock_api):
        ...
        orderbook_message = await self.async_run_with_timeout(self.data_source._order_book_snapshot(self.trading_pair))
    ...
    async def test_get_snapshot(self, mock_api):
        ...
        snapshot_response = await self.async_run_with_timeout(self.data_source.get_snapshot(self.trading_pair))
