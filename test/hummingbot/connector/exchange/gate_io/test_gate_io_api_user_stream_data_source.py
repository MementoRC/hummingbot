...
class TestGateIoAPIUserStreamDataSource(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_listen_for_user_stream_subscribes_to_all_pairs_when_no_trading_pairs(
        ...
        data_source = GateIoAPIUserStreamDataSource(
            self.auth, trading_pairs=[], connector=self.connector, api_factory=self.connector._web_assistants_factory
        )
        ...
        output_queue = asyncio.Queue()
        self.listening_task = self.local_event_loop.create_task(data_source.listen_for_user_stream(output=output_queue))
