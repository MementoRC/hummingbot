...
class KalshiPerpetualDerivativeTests(AbstractPerpetualDerivativeTests.PerpetualDerivativeTests):
    ...
    def _market(self, ticker: str, status: str = "active", contract_size: str = "1.000000") -> dict[str, Any]:
        return {
            "ticker": ticker,
            "status": status,
            "title": f"{contract_size} {self.base_asset}",
    ...
    def funding_info_mock_response(self):
        return {
            "computed_time": _iso(NOW),
            "funding_rate": self.target_funding_info_rate,
    ...
    def _order_status_response(
        self, order: InFlightOrder, fill_count: Decimal, remaining_count: Decimal, last_update_reason: str
    ) -> dict[str, Any]:
        return {
            "order": {
                ...
                "ticker": self.exchange_trading_pair,
                ...
                "created_time": _iso(NOW),
                "last_update_time": _iso(NOW),
    ...
    def configure_erroneous_cancelation_response(
        ...
        url = self._order_url(order)
        mock_api.delete(
            url, status=400, body=self._error_body("invalid_parameters", "invalid parameters"), callback=callback
        )
        return url
    ...
    def _configure_order_status_response(
        self,
        order: InFlightOrder,
    ...
    def _fill(
        ...
        return {
            ...
            "side": "bid" if order.trade_type is TradeType.BUY else "ask",
            "count": str(count),
            "created_time": _iso(created_time),
            "ticker": self.exchange_trading_pair,
            "price": str(price),
    ...
    def configure_partial_fill_trade_response(
        ...
        fill = self._fill(
            order, self.expected_fill_trade_id, self.expected_partial_fill_price, self.expected_partial_fill_amount
        )
    ...
    def configure_erroneous_http_fill_trade_response(
        ...
        mock_api.get(
            self.fills_url, status=400, body=self._error_body("invalid_parameters", "invalid"), callback=callback
        )
        return self.fills_url
    ...
    def _user_order_event(self, order: InFlightOrder, fill_count: Decimal, remaining_count: Decimal):
        return {
            ...
            "msg": {
                ...
                "last_updated_ts_ms": NOW * 1000,
                "order_source": "user",
            },
        }

    def _fill_event(
        self, order: InFlightOrder, trade_id: str, price: Decimal, count: Decimal, fee_cost: str = "30", ts: float = NOW
    ) -> dict[str, Any]:
        return {
            "type": "fill",
            "sid": 1,
        ...
        return {
            ...
            "msg": {
                ...
                "settlement_mark_price": {
                    "price": str(self.target_funding_info_mark_price_ws_updated),
                    "ts_ms": NOW * 1000,
                },
                "funding_rate": {
                    "rate": self.target_funding_info_rate_ws_updated,
    ...
    async def test_update_balances(self, mock_api):
        ...
        self.assertEqual(
            {
                "initial_margin": Decimal("12.5"),
                "maintenance_margin": Decimal("9.615384"),
                "resting_orders_margin": Decimal("3.1"),
            },
            self.exchange._margin_breakdown,
        )
    ...
    def test_set_leverage_failure(self, mock_api):
        ...
        message = (
            f"Kalshi allows at most 6x on {self.trading_pair} (1 / its initial margin rate); requested 20x. "
    ...
    def test_set_leverage_is_rejected_without_kalshi_margin_rates(self):
        ...
        success, message = self.async_run_with_timeout(self.exchange._set_trading_pair_leverage(self.trading_pair, 2))
    ...
    def test_max_leverage_falls_back_to_the_two_sided_estimates_and_is_none_without_estimates(self):
        ...
        no_estimates = {
            **self._market(self.exchange_symbol_for_tokens("ETH", self.quote_asset)),
            "leverage_estimate": None,
            "leverage_estimates": None,
            "long_leverage_estimates": None,
            "short_leverage_estimates": None,
        }
    ...
    def test_budget_checker_caps_order_leverage_at_kalshis(self):
        ...
        def populated(side: TradeType, leverage: int, position_close: bool = False) -> PerpetualOrderCandidate:
            return self.exchange.budget_checker.populate_collateral_entries(
    ...
    def test_available_balance_takes_off_orders_the_last_balance_does_not_include(self, mock_api):
        ...
        self.exchange._initialize_trading_pair_symbols_from_exchange_info(self.all_symbols_request_mock_response)
        mock_api.get(
            _regex(self.balance_url), body=json.dumps(self._balance_response([(0, "2000", "1500")])), repeat=True
        )
        ...
        for order_id, exchange_order_id, trade_type, position_action in (
            ...
            ("13", None, TradeType.SELL, PositionAction.OPEN),
        ):  # still being created
            self.exchange.start_tracking_order(
                order_id=order_id,
    ...
    def test_markets_define_symbols_contract_sizes_and_trading_rules(self):
        ...
        rules = {
            ...
            for rule in self.async_run_with_timeout(self.exchange._format_trading_rules(markets))
        }

        self.assertEqual(
            {"KXBTCPERP": "BTC-USD", "KXDOGEPERP": "DOGE-USD"},
    ...
    def test_create_market_order_is_immediate_limit_order_through_the_book(self, mock_api):
        ...
        mock_api.post(
            self.order_creation_url, body=json.dumps(self.order_creation_request_successful_mock_response), repeat=True
        )
        ...
        self.assertEqual(
            ("10500.0000", "immediate_or_cancel", True), (buy["price"], buy["time_in_force"], buy["reduce_only"])
        )
    ...
    def test_resting_close_order_is_placed_when_refreshed_positions_include_the_position(self, mock_api):
        ...
        mock_api.get(
            _regex(web_utils.private_rest_url(CONSTANTS.POSITIONS_PATH_URL)), body=json.dumps({"positions": [position]})
        )
        mock_api.post(self.order_creation_url, body=json.dumps(self.order_creation_request_successful_mock_response))
    ...
    def test_update_positions_cancels_resting_close_orders_left_without_a_position(self, mock_api):
        ...
        for order_id, trade_type, position_action in (
            ("stale", TradeType.BUY, PositionAction.CLOSE),
        ...
        mock_api.get(
            _regex(web_utils.private_rest_url(CONSTANTS.POSITIONS_PATH_URL)), body=json.dumps({"positions": []})
        )
    ...
    def test_user_stream_ignores_events_for_untracked_orders(self):
        self.exchange._set_current_timestamp(NOW)
        untracked = InFlightOrder(
            client_order_id="99",
            exchange_order_id="liquidation-1",
            trading_pair=self.trading_pair,
            order_type=OrderType.LIMIT,
            trade_type=TradeType.SELL,
            amount=Decimal("1"),
    ...
    def test_rest_fills_add_only_the_fills_the_websocket_missed(self, mock_api):
        ...
        other_order_fill = {
            **self._fill(order, "fill-other", Decimal("10000"), Decimal("1"), created_time=NOW - 50),
            "order_id": "99",
        }
        fills = [
            self._fill(order, "fill-2", Decimal("10000"), Decimal("0.6"), created_time=NOW - 30),
            other_order_fill,
    ...
    def test_rest_fills_of_all_orders_are_fetched_with_a_single_request(self, mock_api):
        ...
        fills = [
            self._fill(first_order, "fill-1", Decimal("10000"), Decimal("0.4"), created_time=NOW - 60),
    ...
    def test_rest_fills_network_error_logs_once_and_the_next_poll_recovers_the_fills(self, mock_api):
        ...
        failures = [
            record for record in self.log_records if record.getMessage().startswith("Failed to fetch trade updates")
        ]
    ...
    def _configure_account_responses(self, mock_api: aioresponses, callback: Callable | None = None):
        position = {
            "subaccount": 0,
            "market_ticker": self.exchange_trading_pair,
            "position": "3.00",
        ...
        mock_api.get(
            _regex(web_utils.private_rest_url(CONSTANTS.POSITIONS_PATH_URL)),
            body=json.dumps({"positions": [position]}),
            callback=callback,
            repeat=True,
        )
        mock_api.get(
            _regex(web_utils.private_rest_url(CONSTANTS.BALANCE_PATH_URL)),
    ...
    def _positions_requests(self, mock_api: aioresponses) -> int:
        return len(
            self._all_executed_requests(mock_api, _regex(web_utils.private_rest_url(CONSTANTS.POSITIONS_PATH_URL)))
        )
    ...
    def test_fills_of_untracked_orders_also_refresh_positions(self, mock_api):
        ...
        self._configure_account_responses(mock_api)
        liquidation = {
            **self._fill_event(order, "t-1", Decimal("7.7825"), Decimal("3.00"))["msg"],
            "order_id": "liquidation-1",
            "client_order_id": "",
        }
    ...
    def test_fetch_last_fee_payment_queries_recent_funding_history(self, mock_api):
        ...
        timestamp, rate, amount = self.async_run_with_timeout(self.exchange._fetch_last_fee_payment(self.trading_pair))

        self.assertEqual(
            (self.target_funding_payment_timestamp, Decimal("100"), Decimal("200")), (timestamp, rate, amount)
        )
        request = self._all_executed_requests(mock_api, self.funding_payment_url)[0]
        self.assertEqual(
            {"ticker": self.exchange_trading_pair, "start_date": "2022-07-05", "end_date": "2022-07-06"},
            request.kwargs["params"],
        )

    def test_not_found_errors_are_recognized(self):
        not_found = IOError(
            "Error executing request DELETE https://x. HTTP status is 404. "
            'Error: {"error":{"code":"not_found","message":"not found"}}'
        )
        other = IOError(
            "Error executing request DELETE https://x. HTTP status is 400. "
        ...
        self.assertTrue(
            any(
                ...
                and record.getMessage().startswith(
                    f"Error fetching status update for the active order {order.client_order_id}"
                )
    ...
    def test_time_synchronizer_related_request_error_detection(self):
        self.assertFalse(
            self.exchange._is_request_exception_related_to_time_synchronizer(
