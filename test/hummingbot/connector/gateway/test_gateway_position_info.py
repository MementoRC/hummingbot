...
class GatewayPositionInfoContractTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_position_specific_404_returns_none(self):
        ...
        fake = _fake_connector(ValueError("Gateway error: Position not found or closed: pos123 (Not Found) [code: X]"))
    ...
    async def test_position_specific_gateway_error_404_returns_none(self):
        ...
        fake = _fake_connector(GatewayError("Position not found or closed: pos123", status=404, http_error="Not Found"))
        result = await Gateway.get_position_info_fresh(
            fake, trading_pair="SOL-USDC", dex_name="orca", trading_type="clmm", position_address="pos123"
        )
        self.assertIsNone(result)
    ...
    async def test_position_wording_on_non_404_raises(self):
        ...
        fake = _fake_connector(GatewayError("Failed to load position: position not found in account data", status=500))
    ...
    async def test_unrelated_gateway_error_404_raises(self):
        fake = _fake_connector(GatewayError("Wallet not found: test_wallet", status=404, http_error="Not Found"))
        with self.assertRaises(GatewayError):
            await Gateway.get_position_info_fresh(
                fake, trading_pair="SOL-USDC", dex_name="orca", trading_type="clmm", position_address="pos123"
            )
