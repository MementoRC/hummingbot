...
class ConnectorSettingTests:
    def test_connector_setting_creates_non_trading_connector_instance(self):
        ...
        assert "binance" == connector.name
        assert "com" == connector._domain
        assert "" == connector._auth.api_key
        assert "" == connector._auth.secret_key
        assert not connector._trading_required
