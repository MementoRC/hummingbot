...
def _simulation(
    config: PositionExecutorConfig, timestamps, close_type=CloseType.TIME_LIMIT, net_pnl_quote=Decimal("3")
) -> ExecutorSimulation:
    df = pd.DataFrame(
        {
...
class TestFinalTickLedger(unittest.IsolatedAsyncioTestCase):
    ...
    def _engine(controller, simulations_by_id):
        ...
        engine.simulate_executor = MagicMock(side_effect=lambda config, df, trade_cost: simulations_by_id[config.id])
        return engine
    ...
    def test_executor_stopped_after_the_last_look_keeps_its_terminated_info(self):
        ...
        engine.handle_stop_action(StopExecutorAction(controller_id="test", executor_id="stopped_late"), timestamp=120.0)
