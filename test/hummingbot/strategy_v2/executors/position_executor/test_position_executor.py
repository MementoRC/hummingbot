...
class TestPositionExecutor(IsolatedAsyncioWrapperTestCase):
    ...
    def test_cum_fees_includes_take_profit_limit_order(self, _, trading_rules_mock):
        ...
        open_order.order.update_with_trade_update(
            TradeUpdate(
                trade_id="1",
                client_order_id="OID-BUY-1",
                exchange_order_id="EOID1",
        ...
        tp_order.order.update_with_trade_update(
            TradeUpdate(
                trade_id="2",
                client_order_id="OID-SELL-TP",
                exchange_order_id="EOID2",
    ...
    def test_take_profit_limit_order_uses_open_action_in_oneway(self):
        executor = self._make_perpetual_executor(PositionMode.ONEWAY)
        with patch.object(PositionExecutor, "amount_to_close", new_callable=PropertyMock, return_value=Decimal("1")):
