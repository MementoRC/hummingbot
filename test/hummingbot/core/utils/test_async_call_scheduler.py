...
from hummingbot.core.utils.async_call_scheduler import (
    AsyncCallScheduler,
    AsyncCallSchedulerItem,
    AsyncCallSchedulerTimeoutError,
)


class TestAsyncCallScheduler(IsolatedAsyncioWrapperTestCase, LoggerMixinForTest, ):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        # Done on purpose to verify the EventLoop insensitivity of the AsyncCallScheduler
    ...
    async def test_schedule_async_call_with_exception(self):
        ...
        self.assertEqual(type(AsyncCallSchedulerTimeoutError()), type(results[2]), "Coroutine 3 did not raise a AsyncCallSchedulerError")
        self.assertIsInstance(results[3], ValueError, "Coroutine 4 should have raised a ValueError")
    ...
    async def test_test_stop_and_restart(self):
        ...
        # This restarts the scheduler task
    ...
    async def test_coro_scheduler_out_timeout(self):
        ...
        if not fut.cancelled():
            self.assertEqual(type(AsyncCallSchedulerTimeoutError()), type(results[0]),
    ...
    async def test_coro_scheduler_logging_message_on_timeout(self):
        ...
        delay: float = 0.5
        timeout: float = delay - 0.1
        error_message: str = "Scheduled coroutine timed out:"
        ...
        self.assertTrue(self.is_partially_logged("WARNING", error_message))
        # We cancelled the scheduler, so we should see a log message about that
        self.assertTrue(self.is_partially_logged("ERROR", "Scheduler received cancel signal"))
    ...
    async def test_coro_scheduler_sleep_exception(self):
        ...
        with patch("asyncio.sleep", side_effect=sleep_side_effect):
            asyncio.create_task(acs._coro_scheduler(coro_queue, interval=delay / 1.5))
            await asyncio.wait_for(asyncio.gather(fut, return_exceptions=True), timeout=delay * 1.5)

        self.assertTrue(fut.exception(), "Sleep failed")
        self.assertTrue(self.is_partially_logged("WARNING",
                                                 "Coroutine Timeout Error [[Got exception: Sleep failed]]"))
    ...
    async def test_cancel_scheduled_task(self):
        ...
        acs = AsyncCallScheduler()
        acs.start()
        task = asyncio.create_task(acs.call_async(set_flag))

        # Task not started yet
        self.assertFalse(flag[0], "Task was incorrectly started")
        # Release to the EV Loop
        await asyncio.sleep(delay / 2)
        # Task started
        self.assertTrue(flag[0], "Task was not correctly started")
        # Await for the task to be finished
        await asyncio.sleep(delay / 2)
        # Task completed
        self.assertFalse(flag[0], "Task was not completed")
        await asyncio.sleep(0.1)
        self.assertTrue(task.done(), "Task was not marked as done")

        # Same experiment with a cancel mid-way
        task = asyncio.create_task(acs.call_async(set_flag))
        # Task not started yet
        self.assertFalse(flag[0], "Task was incorrectly started")
        # Release to the EV Loop
        ...
        self.assertTrue(flag[0], "Task was not cancelled")
        await asyncio.sleep(0.1)
        self.assertTrue(task.done(), "Task was not marked as done")
